...
pub struct ProtocolSettings {
    ...
    pub offline_sync_limit: Option<aether_core::OfflineSyncLimit>,
}
...
<soap:Body>
  <FindFolder xmlns="http://schemas.microsoft.com/exchange/services/2006/messages" Traversal="Shallow">
    ...
    if let Some(aether_core::OfflineSyncLimit::Days(days)) = settings.offline_sync_limit {
...
fn fetch_recent_imap(
    ...
    let sequence = if let Some(aether_core::OfflineSyncLimit::Days(days)) = settings.offline_sync_limit {
        let after_date = Utc::now() - chrono::Duration::days(days as i64);
