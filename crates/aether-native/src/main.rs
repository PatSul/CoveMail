...
use aether_calendar::{CalendarService, CalendarSettings};
...
use aether_email::{EmailService, OutgoingAttachment, OutgoingMail, ProtocolSettings};
use aether_security::{OAuthWorkflow, SecretKey, SecretStore};
use aether_storage::Storage;
use aether_tasks::{TaskService, TaskSettings};
use anyhow::Context;
use base64::Engine;
...
struct OAuthDraft {
    ...
    started: bool,
    sync_limit: aether_core::OfflineSyncLimit,
}
    ...
    fn initialize() -> anyhow::Result<Self> {
        ...
        Ok(Self {
            ...
            oauth: OAuthDraft {
                ...
                started: false,
                sync_limit: aether_core::OfflineSyncLimit::Days(30),
            },
    ...
    fn show_setup_wizard(&mut self, ctx: &egui::Context) {
        egui::CentralPanel::default().show(ctx, |ui| {
            ui.vertical_centered(|ui| {
                ui.add_space(80.0);
                ui.heading(egui::RichText::new("Welcome to AegisInbox").size(36.0).strong());
                ui.add_space(10.0);
            ...
            View::Security => {
                ...
                ui.horizontal(|ui| {
                    let label = match self.oauth.sync_limit {
                        aether_core::OfflineSyncLimit::All => "All Time".to_string(),
                        aether_core::OfflineSyncLimit::Days(d) => format!("Last {d} Days"),
                    };
                    egui::ComboBox::from_label("Offline Sync Limit")
                        .selected_text(label)
                        .show_ui(ui, |ui| {
                            ui.selectable_value(&mut self.oauth.sync_limit, aether_core::OfflineSyncLimit::Days(30), "Last 30 Days");
                            ui.selectable_value(&mut self.oauth.sync_limit, aether_core::OfflineSyncLimit::Days(90), "Last 90 Days");
                            ui.selectable_value(&mut self.oauth.sync_limit, aether_core::OfflineSyncLimit::All, "All Time");
                        });
                });
...
fn oauth_profile_for_provider(
    ...
    client_id: &str,
    redirect_url: &str,
) -> Result<aether_core::OAuthProfile, String> {
